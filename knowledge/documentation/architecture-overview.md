...
2. **Deviation judging** (REST, not WS): client posts the reply plus the on-screen chunk -> `deviation_judge` (one DeepSeek call) -> on `deviated`, write to `deviation-index` (`add_docs` or `create_index` if absent) -> `load_index` reload so it is queryable on the next transcript update. `followed` stores nothing.
...
## Operational notes
- Required env: `MOSS_PROJECT_ID`, `MOSS_PROJECT_KEY`. Optional: `DEEPSEEK_API_KEY` (LLM features), a Deepgram key (voice; see `voice_stream.py`), `DEMO_PASSWORD`, `MOSS_CACHE_DIR`.
